...
def create_risk_plot(df, source, width, x, threshold, test_sample):
    p = figure(title='MethylScoreAML', width=width, height=300,
               tools="xbox_select,reset,save", active_drag='xbox_select',
               x_axis_label=x, y_axis_label="Patient Percentile")
    p.toolbar.logo = None

    p.quad(left=-4.15, right=threshold, bottom=0, top=0.6, color="#1f77b4", level="underlay", alpha=0.2)
    p.quad(left=threshold, right=0.65, bottom=0, top=0.6, color="#ff7f0e", level="underlay", alpha=0.2)
    ...
    scatter = p.circle(x, 'Percentile', source=source, color="steelblue", alpha=0.4, 
    ...
    if test_sample:
        ...
        p.star(x=df.loc[test_sample][x], y=0, size=15, color="black", alpha=0.9, 
               legend_label=f'{test_sample}, {df.loc[test_sample]["MethylScoreAML Categorical"]} MethylScoreAML Categorical ({df.loc[test_sample][x]:.2f})',
...
def create_histogram_plot(df, source, width, x):
    p = figure(title='MethylScoreAML - Select clusters on the map and their prognosis will appear here', 
               width=width, height=300, x_axis_label=x, y_axis_label='Frequency', tools="save,reset")
    p.toolbar.logo = None

    hist, edges = np.histogram(df[x], bins=50, range=[-4.15, 0.65])
    ...
    callback = CustomJS(args=dict(source=source, hist_source=hist_source, edges=edges, p=p), code="""
        ...
        for (let i = 0; i < indices.length; i++) {
            const idx = indices[i];
            const value = data['MethylScoreAML'][idx];
            sum_p_death += value;
...
def create_race_risk_plot(df, source, width):
    race_risk_counts = df.groupby(['Race or ethnic group', 'MethylScoreAML Categorical']).size().unstack().fillna(0)
    ...
    p = figure(title='Race or Ethnic Group by MethylScoreAML Categorical', width=width, height=300,
    ...
    p.vbar_stack(['low', 'high'], x='race', width=0.7, color=["#1f77b4", "#ff7f0e"], source=race_hist_source,
                 legend_label=['Low MethylScoreAML Categorical', 'High MethylScoreAML Categorical'], line_color="white", alpha=0.5)
    ...
    callback = CustomJS(args=dict(source=source, race_hist_source=race_hist_source, race_categories=race_categories), code="""
        ...
        for (let i = 0; i < indices.length; i++) {
            ...
            const risk = data['MethylScoreAML Categorical'][idx];
...
def plot_alma(df, test_sample=None, 
              ...
              cols=['AL Epigenomic Subtype','WHO 2022 Diagnosis','Hematopoietic Entity', 
                    'Vital Status', 'MethylScoreAML Categorical', 
