...
from bokeh.models import (
    ColumnDataSource, DataTable, TableColumn, CategoricalColorMapper, HoverTool,
    Label, Span, GroupFilter, CDSView, Legend, LegendItem, Tabs, TabPanel,
    CustomJS, FactorRange, Button
)
...
def create_data_table(df, cols, width):
    columns = [TableColumn(field=col, title=col) for col in cols + ['Gene Fusion', 'Karyotype']]
    return DataTable(source=ColumnDataSource(df), columns=columns, editable=True, width=width, index_position=None, height=300)
...
def create_histogram_plot(df, source, width, x):
    ...
    hist, edges = np.histogram(df[x], bins=50, range=[0.2, 0.8])
...
def plot_alma(df, table=True, test_sample=None, 
              xaxis="PaCMAP 1 of 2", yaxis="PaCMAP 2 of 2",
              x_range=(-45, 40), y_range=(-50, 45), 
              cols=['WHO 2022 Diagnosis','AL Epigenomic Subtype', 'Hematopoietic Entity', 
    ...
    data_table = create_data_table(df, cols, width) if table else None
